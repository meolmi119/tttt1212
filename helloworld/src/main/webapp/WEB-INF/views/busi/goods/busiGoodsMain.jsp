...
function search_goods_list(fixedSearchPeriod){
    ...
    formObj.method="get";
    formObj.action="${contextPath}/busiGoodsMain.do";
    formObj.submit();
}
   ...
   <c:choose>
     ...
     <c:forEach var="item" items="${newGoodsList }">
			 <TR>       
				...
				<TD >
				 <a href="${pageContext.request.contextPath}/modifyGoodsStayForm.do?goods_id=${item.goods_id}">
				    <strong>${item.goods_stay_name } </strong>
				 </a> 
				</TD>
				<TD>
				<strong>${item.goods_stay_sort }</strong> 
				</TD>
				<TD >
				   <strong>${item.goods_stay_room_number }</strong> 
				</TD>
				<td>
				  <strong>${item.goods_stay_num_people }</strong>
				</td>
				<td>
				 <strong>${item.goods_stay_price }원, ${item.goods_stay_sales_price }</strong> 
				</td>
				<%-- <td>
           ...
           <tr>
             <td colspan=8 class="fixed">
		         ...
		         <c:if test="${section >1 && page==1 }">
		          <a href="${contextPath}/busiGoodsMain.do?chapter=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp; &nbsp;</a>
		         </c:if>
		          <a href="${contextPath}/busiGoodsMain.do?chapter=${section}&pageNum=${page}">${(section-1)*10 +page } </a>
		         <c:if test="${page ==10 }">
		          <a href="${contextPath}/busiGooodsMain.do?chapter=${section+1}&pageNum=${section*10+1}">&nbsp; next</a>
		         </c:if> 
	      		</c:forEach> 
...
<H3>숙박상품등록하기</H3>
<DIV id="search">
	<form action="${contextPath}/addNewGoodsStayForm.do">
		<input   type="submit" value="숙박 상품 등록하기">
	</form>
</DIV>
</body>
</html>