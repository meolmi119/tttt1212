...
<div id="busi_order_history_list">
	...
	<table id="busi_order_history_stay">
		<thead>
			<tr>
				<td colspan="8" style="background:white;"><strong>상품 결제 현황</strong></td>
			</tr>
			<tr>
				<td>숙소 이름</td>
				<td>결제ID</td>
		...
		<c:choose>
			...
			<c:when test="${ empty busiOrderList }">
			<tr>
			...
			<c:otherwise>
				<c:forEach var="item" items="${busiOrderList }">
				<tr>
					...
					<td>
						...
						<c:choose>
							...
							<c:when test="${ item.stay_order_state == '취소처리중' }">
								<a href="${contextPath }/order/busiPermitStayState.do?stay_seq_num=${item.stay_seq_num}">
